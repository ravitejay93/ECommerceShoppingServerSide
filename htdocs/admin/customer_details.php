...
<?php 
require "conn.php";
$result = mysqli_query($localhost,"select  * from customer" )or die(mysqli_error());
$result2 = mysqli_query($localhost,"select * from address_user") or die(mysqli_error());
 if (mysqli_num_rows($result) > 0) {
    
    while ($row = mysqli_fetch_array($result)) {  ?>
       <tr>     
          <td><?php echo $row['customer_id'];
		              $temp=$row['customer_id'] ;?></td>
         <td><?php    echo $row['first_name']; ?></td>
         <td><?php    echo $row['last_name']; ?></td>
          <td><?php   echo $row['phone']; ?></td>
            <td><?php echo $row['email']; ?></td>
			<?php
			    $result1 = mysqli_query($localhost,"select * from user where user_id = $temp ")or die(mysqli_error($localhost));
				$row1=mysqli_fetch_array($result1);?>
			 <td><?php echo $row1['password'];?></td>
         <td><?php    echo $row1['create_time']; ?></td>
		 <?php
			    $result2 = mysqli_query($localhost,"select * from address_user where personal_id=$temp ")or die(mysqli_error());
         ...
         <td><?php    echo $row2['zip']; ?></td>
                       ...
                       <td> <?php 
             echo '<a href="delete.php?id='.$temp.'">Delete</a>';?></td></tr>
